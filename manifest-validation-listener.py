...
def create_aspera_dir(submission_dirpath, access_level):
    """
    Create an Aspera directory for the submitted data.
    """
    sub_logger.debug("In create_aspera_dir().")
    ...
    # Create an Aspera directory for the submission
    try:
        sub_logger.info(f"Making submission directory {submission_dirpath}")
        orig_mask = os.umask(0)
        try:
            if is_restricted:
                os.makedirs(submission_dirpath, mode=0o0770)
            else:
                os.makedirs(submission_dirpath, mode=0o0775)
        finally:
            os.umask(orig_mask)
    except Exception as err:
        sub_logger.exception(f"Error in mkdir {submission_dirpath}: {err}",
                             exc_info=True)

    # If restricted, change group to nemorestricted (GID 10038)
    try:
        sub_logger.info(
            f"Changing owner for submission directory {submission_dirpath}"
        )
        orig_mask = os.umask(0)
        try:
            if is_restricted:
                os.chown(submission_dirpath, -1, 10038)
            else:
                os.chown(submission_dirpath, -1, 10016)
        finally:
            os.umask(orig_mask)
    except Exception as err:
        sub_logger.exception(f"Error in chown {submission_dirpath}: {err}",
                             exc_info=True)
...
def get_submission_dirpath(submitter_username, submission_id):
    """
    Returns the submission directory's full path.
    This transformed the submitter's Aspera docroot into a path accessible
    to the Ingest service account and appends the submission ID to it.
    """
    sub_logger.debug("In get_submission_dirpath().")
    nemo_root = config['general']['nemo-root']

    submission_docroot = get_submission_docroot(submitter_username)
    sub_logger.info(f"Submission docroot is: {submission_docroot}")

    if "restricted" in submission_docroot:
        access_dir = "restricted"
    else:
        access_dir = "public"

    # Get subdirectories that follow 'brain'. An empty string for public submissions.
    subdirs = submission_docroot.split("brain")[-1]

    # Remove leading "/", otherwise it breaks submission dirpath building
    if subdirs.startswith("/"):
        subdirs = subdirs[1:]

    submission_dirpath = os.path.join(
                                nemo_root,
                                access_dir,
                                "incoming/brain",
                                subdirs,
    ...
    sub_logger.info(f"Submission dirpath is: {submission_dirpath}")

    return submission_dirpath
...
def handle_valid_message(data):
    ...
    submission_id = data['submission_id']
    gcp_manifest_path = data['gcp_manifest_path']
    ...
    # Download the manifest from GCP to IGS filesystem
    try:
        ...
        error_message = "Unable to download validated manifest from " + \
                        f"{gcp_manifest_path} due to error: "
        sub_logger.exception(error_message, exc_info=True)
        error_message += "\n"
        error_message += traceback.format_exc()
        send_misc_error_email(submission_id, gcp_manifest_path, error_message, dryrun)
        return
    ...
    if not os.path.isfile(vocab_to_dirname_filepath):
        ...
        send_misc_error_email(submission_id, gcp_manifest_path, msg, dryrun)
        return
        ...
        try:
            if dryrun:
                ...
                if program == 'bican':
                    ...
                    access_level = "restricted"
                elif program == 'scorch' and gcp_project_name == 'scorch':
                    # Copy manifest into valid manifest area.
                ...
                # Get the submissions's Aspera directory path
                submission_dirpath = get_submission_dirpath(submitter_username, submission_id)

                # Create the Aspera directory for the submission
                create_aspera_dir(submission_dirpath, access_level)
